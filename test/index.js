...
describe('github', () => {
  ...
  it('symbolic link', async () => {
    const path = 'https://github.com/oraclize/ethereum-api/oraclizeAPI.sol'
    console.time('first time')
    let content = await github.parser(path)
    console.timeEnd('first time')
    content.should.be.a('string')
  ...
  it('no found', async () => {
    const path =
      'https://github.com/OpenZeppelin/openzeppelin-solidity/contracts/math/SafeMath2.sol'
    try {
      await github.parser(path)
    } catch (error) {
  ...
  it('path resolve', async () => {
    const path =
      'https://github.com/OpenZeppelin/openzeppelin-solidity/contracts/examples/SimpleToken.sol'
    let content = await github.parser(path)
